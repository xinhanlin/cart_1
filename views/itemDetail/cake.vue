<template>
  ...
  <div class="content-wrapper">
    ...
    <div class="content">
      <div>品名:{{ $store.state.data.cake[`${n}`].cartItem }}</div>
      <div>價格:{{ $store.state.data.cake[`${n}`].price }}</div>
      <div>
      ...
      <div>
        <button
          v-if="storeCartNo.includes($store.state.data.cake[`${n}`].no)"
          class="add-cart add-cart-delete"
          @click="deletcake($store.state.data.cake[`${n}`].no)"
        >
          刪除商品
        </button>
        <button
          v-else
          @click="addcake($store.state.data.cake[`${n}`].no)"
          class="add-cart"
        >
          加入購物單
        </button>
...
export default {
  ...
  setup() {
    ...
    return { n, addcake, deletcake, storeCartNo };
  },
};
</script>
<style scoped>
