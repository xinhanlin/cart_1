<template>
  <div class="item-outerwrapper">
    <div
      v-for="item in $store.state.data.drink"
      :key="item.no"
      class="item-wrapper"
    >
      <img
      ...
      <ul class="item">
        ...
        <li class="item-bottom">
          <router-link :to="`/drink/${item.id}`" class="item-detail"
            >詳情</router-link
          >
          <div>
            <button
              ...
              @click="deletdrink(item.no)"
            >
              刪除商品
            </button>
            <button v-else @click="adddrink(item.no)" class="add-cart">
              加入購物單
            </button>
          </div>
        </li>
...
export default {
  setup() {
    ...
    return { adddrink, storeCartNo, deletdrink };
  },
};
</script>
<style scoped>
